          ...
          function displayWeatherDetails(response) {
            ...
            let timeChange = document.querySelector("#getCurrentTime");
            timeChange.innerHTML = day + " " + hour + ":" + correctMinute(minute);
            document.querySelector("#icon").setAttribute("src", `https://openweathermap.org/img/wn/${response.data.weather[0].icon}@2x.png`);
            document.querySelector("#icon").setAttribute("alt", response.data.weather[0].description);
          weatherForecast(response.data.coord);
        }

        //let time = document.querySelector("#updateWind");
        //time.innerHTML = new Date((response.data.wind.speed)*1000);
        //document.querySelector("#getCurrentTime").innerHTML = response.data.dt;
